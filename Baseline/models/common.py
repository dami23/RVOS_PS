...
class ResBlock(nn.Module):  # modified
...
class Bottleneck(nn.Module):    # generally used in resnet101
...
# 对target frame进行 self-attention
class ConvSA(nn.Module):
    def __init__(self, C, emb=512): # C = 2048
        super(ConvSA, self).__init__()
        self.Query = nn.Conv2d(C, emb, kernel_size=(3,3), padding=(1,1), stride=1)  # input channel:2048, output channel:512
    ...
    def forward(self, feat): # (B, C, H, W), 传入vis
        B, C, H, W = feat.size()
        query = self.Query(feat)
        key = self.Key(feat)
        value = self.Value(feat)
        
        # 将数组重组，-1代表自动调整为B*H*W*C/B*H*W=C
        query_ = query.reshape(B, -1, H*W) # (B,C,H*W)
...
# 对语义信息进行self-attention    
class LinearSA(nn.Module):
    
    def __init__(self, C, emb=512):     # C = 1024, emb = 512
...
class Refine(nn.Module):
    ...
    def forward(self, f, pm):   # f:(2,1024,20,20), pm:(2,256,10,10)
        s = self.ResFS(self.convFS(f))  # convFS(f)-->(2,256,20,20), s:(2,256,20,20)
        m = s + F.interpolate(pm, scale_factor=self.scale_factor, mode='nearest')   # F.interpolate()-->(2,256,20,20)
        m = self.ResMM(m)
        return m
...
def pad_divide_by(in_list, d, in_size):
    out_list = []
    h, w = in_size  # 如(frame.size()[2], frame.size()[3])
    if h % d > 0:
        new_h = h + d - h % d
    else:
        new_h = h
    ...
    lh, uh = int((new_h-h) / 2), int(new_h-h) - int((new_h-h) / 2)
    lw, uw = int((new_w-w) / 2), int(new_w-w) - int((new_w-w) / 2)
    pad_array = (int(lw), int(uw), int(lh), int(uh))    # 定义了四个参数，对输入矩阵的后两个维度进行扩充
    for inp in in_list:
        out_list.append(F.pad(inp, pad_array))  # F.pad()用以扩充维度
    return out_list, pad_array  # 返回扩充维度后的frame或mask

####### self-attention layer in Decoder #######
class Self_Attention(nn.Module):
    def __init__(self, indim):
        super(Self_Attention, self).__init__()
        self.channel_in = indim
        
        self.query_conv = nn.Conv2d(in_channels = indim, out_channels = indim, kernel_size=1)
        self.key_conv = nn.Conv2d(in_channels = indim, out_channels = indim, kernel_size=1)
        self.value_conv = nn.Conv2d(in_channels = indim, out_channels = indim, kernel_size=1)
        # self.Query = nn.Conv2d(C, emb, kernel_size=(3,3), padding=(1,1), stride=1)
        # self.Key = nn.Conv2d(C, emb, kernel_size=(3,3), padding=(1,1), stride=1)
        # self.Value = nn.Conv2d(C, emb, kernel_size=(3,3), padding=(1,1), stride=1)
        
    def forward(self, m5, r4, r3, r2):
        # m5:(2,256,10,10)
        # r4:(2,256,20,20)
        # r3:(2,256,40,40)
        # r2:(2,256,80,80)
        m5_ = F.interpolate(m5, scale_factor=8, mode='nearest') # resize to (2,256,80,80)
        r4_ = F.interpolate(r4, scale_factor=4, mode='nearest')
        r3_ = F.interpolate(r3, scale_factor=2, mode='nearest')
        # feature_value_1 = torch.add(m5_, r4_)
        # feature_value_2 = torch.add(r3_, r2)
        # feature_value = torch.stack((m5_,r4_,r3_,r2),dim=1)
        feature_value = m5_ + r4_ + r3_ + r2
        
        B, C, H, W = feature_value.size()
        
        query = self.query_conv(feature_value)
        key = self.key_conv(feature_value)
        value = self.value_conv(feature_value)
